...
void printall()
{
    ...
    stringer="""
	        ...
	        //delay();
	        Xil_Out32(XPAR_MYIP_AXI_LUD_WRAPPER_0_BASEADDR + 4*({0}+i), 1); //Making enable 1
	        //delay();
	        Xil_Out32(XPAR_MYIP_AXI_LUD_WRAPPER_0_BASEADDR + 4*({1}+i), 0); //Making write enable 0
	        //delay();
	        val = Xil_In32(XPAR_MYIP_AXI_LUD_WRAPPER_0_BASEADDR + 4*({2}+i)); //Reading from dout
	        val_float = int_to_float(val);
	        //delay();
	        Xil_Out32(XPAR_MYIP_AXI_LUD_WRAPPER_0_BASEADDR + 4*({0}+i), 0); //Making enable 0
"""
    SW.write(stringer.format(s_en,s_we,s_dout))
    SW.write("""
	        //delay();
