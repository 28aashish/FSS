...
delay_FPGALoad_INST();
for(j=0;j<sub_instructions;j++){
val = Xil_In32(XPAR_MYIP_AXI_LUD_WRAPPER_0_BASEADDR + 4*(30+j)); //Reading from BRAM
delay_FPGALoad_INST();
if(val != 0){
...
delay_FPGALoad_INST();
for(j=0;j<sub_instructions;j++){
val_int = Xil_In32(XPAR_MYIP_AXI_LUD_WRAPPER_0_BASEADDR + 4*(30+j)); //Reading from BRAM
delay_FPGALoad_INST();
if(val_int != Inst[i][j]){
