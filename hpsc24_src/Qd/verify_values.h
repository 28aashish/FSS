...
for(i=0;i<4;i++){
    ...
    for(i=0;i<4;i++){
    	for(j = 0; j < tmp;j++){
	        ...
	        Xil_Out32(XPAR_MYIP_LUDECOMPOSITION_V2_V1_0_0_BASEADDR + 4*(4+i), j); //Writing address
	        //delay();
	        Xil_Out32(XPAR_MYIP_LUDECOMPOSITION_V2_V1_0_0_BASEADDR + 4*(16+i), 1); //Making enable 1
	        //delay();
	        val = Xil_In32(XPAR_MYIP_LUDECOMPOSITION_V2_V1_0_0_BASEADDR + 4*(12+i)); //Reading from dout
	        //delay();
	        Xil_Out32(XPAR_MYIP_LUDECOMPOSITION_V2_V1_0_0_BASEADDR + 4*(16+i), 0); //Making write enable 0
	        //delay();
        ...
        for(j=0;j<tmp;j++){
            if(U[i][j] != -11110000){//It indicates that, that value of L does not exist
                ...
                if(U[i][j] != 0){
                    ...
                    if(percent_error>5 || percent_error<-5){
                        invalid_values = invalid_values + 1;
                        printf("(i,j)=%d %d p%f  U[%d][%d] = %f, BRAM_dump = %f\n",i,j,percent_error,row_ind[i][j],col_ind[i][j],L[i][j],bram_dump[i][j]);
